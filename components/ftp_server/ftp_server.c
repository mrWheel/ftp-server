...
typedef struct { char base[FTP_PATH_MAX], user[64], pass[64]; uint16_t port, pmin, pmax, next_port; size_t bufsize, max_clients; uint32_t ctimeout, dtimeout; bool anon; } server_cfg_t;
typedef struct { int ctrl, pasv; char cwd[FTP_PATH_MAX], rename_from[FTP_PATH_MAX], login_user[64]; bool authed; off_t rest; } session_t;
...
static int send_all(int fd,const void *data,size_t n){ const char *p=data; while(n){ ssize_t k=send(fd,p,n,0); if(k<0&&errno==EINTR)continue; if(k<=0)return -1; p+=k;n-=k;}return 0; }
static int reply(session_t*s,int code,const char*fmt,...){ char text[384],line[420];va_list ap;va_start(ap,fmt);vsnprintf(text,sizeof(text),fmt,ap);va_end(ap);int n=snprintf(line,sizeof(line),"%d %s\r\n",code,text);return n>0&&n<(int)sizeof(line)?send_all(s->ctrl,line,n):-1;}
static bool normalize(const char *cwd,const char *arg,char*out,size_t cap){ char tmp[FTP_PATH_MAX],work[FTP_PATH_MAX],*save=NULL; if(!arg||!*arg)snprintf(tmp,sizeof(tmp),"%s",cwd);else if(arg[0]=='/')snprintf(tmp,sizeof(tmp),"%s",arg);else snprintf(tmp,sizeof(tmp),"%s%s%s",cwd,strcmp(cwd,"/")?"/":"",arg); out[0]='/';out[1]=0;snprintf(work,sizeof(work),"%s",tmp);for(char*t=strtok_r(work,"/",&save);t;t=strtok_r(NULL,"/",&save)){if(!strcmp(t,".")||!*t)continue;if(!strcmp(t,"..")){char*p=strrchr(out,'/');if(p&&p!=out)*p=0;else out[1]=0;continue;}if(strchr(t,'\\'))return false;size_t l=strlen(out),q=strlen(t);if(l+q+2>cap)return false;if(l>1)strcat(out,"/");strcat(out,t);}return true;}
...
static int open_data(session_t*s){if(s->pasv<0)return -1;int fd=accept(s->pasv,NULL,NULL);closefd(&s->pasv);if(fd>=0)timeout_fd(fd,g.dtimeout);return fd;}
...
static int listing(session_t*s,const char*arg,bool names){char v[FTP_PATH_MAX],p[FTP_PATH_MAX];if(!paths(s,arg&&*arg?arg:NULL,v,p))return reply(s,550,"Invalid path.");DIR*d=opendir(p);if(!d)return reply(s,550,"Directory unavailable.");reply(s,150,"Opening data connection.");int data=open_data(s);if(data<0){closedir(d);return reply(s,425,"Cannot open data connection.");}struct dirent*e;char line[768],full[FTP_PATH_MAX+256+2];while((e=readdir(d))){if(!strcmp(e->d_name,".")||!strcmp(e->d_name,".."))continue;if(names)snprintf(line,sizeof(line),"%s\r\n",e->d_name);else{snprintf(full,sizeof(full),"%s/%s",p,e->d_name);struct stat st={0};stat(full,&st);struct tm tm={0};time_t when=st.st_mtime?st.st_mtime:time(NULL);localtime_r(&when,&tm);char date[32];strftime(date,sizeof(date),"%b %d %H:%M",&tm);snprintf(line,sizeof(line),"%crw-r--r-- 1 ftp ftp %10lld %s %s\r\n",S_ISDIR(st.st_mode)?'d':'-',(long long)(S_ISDIR(st.st_mode)?0:st.st_size),date,e->d_name);}if(send_all(data,line,strlen(line)))break;}closedir(d);closefd(&data);return reply(s,226,"Transfer complete.");}
static int transfer(session_t*s,const char*arg,bool upload){char v[FTP_PATH_MAX],p[FTP_PATH_MAX];if(!paths(s,arg,v,p))return reply(s,550,"Invalid path.");int f=open(p,upload?(O_WRONLY|O_CREAT|(s->rest?0:O_TRUNC)):O_RDONLY,0644);if(f<0){storage_report(upload?"STOR":"RETR",p,-1);return reply(s,550,"File unavailable.");}if(s->rest&&lseek(f,s->rest,SEEK_SET)<0){close(f);s->rest=0;storage_report(upload?"STOR":"RETR",p,-1);return reply(s,550,"Invalid restart position.");}s->rest=0;reply(s,150,"Opening binary data connection.");int data=open_data(s);if(data<0){close(f);storage_report(upload?"STOR":"RETR",p,-1);return reply(s,425,"Cannot open data connection.");}char*b=malloc(g.bufsize);bool ok=b;while(ok){ssize_t n=upload?recv(data,b,g.bufsize,0):read(f,b,g.bufsize);if(n==0)break;if(n<0){if(errno==EINTR)continue;ok=false;break;}if(upload){char*q=b;ssize_t left=n;while(left){ssize_t k=write(f,q,left);if(k<0&&errno==EINTR)continue;if(k<=0){ok=false;break;}q+=k;left-=k;}}else if(send_all(data,b,n)){ok=false;break;}}free(b);if(upload)fsync(f);close(f);closefd(&data);storage_report(upload?"STOR":"RETR",p,ok?0:-1);return reply(s,ok?226:426,ok?"Transfer complete.":"Transfer aborted.");}
static bool need_auth(const char*c){return strcmp(c,"USER")&&strcmp(c,"PASS")&&strcmp(c,"QUIT")&&strcmp(c,"NOOP")&&strcmp(c,"SYST")&&strcmp(c,"FEAT");}
static bool command(session_t*s,char*line){char*arg=strchr(line,' ');if(arg){*arg++=0;while(*arg==' ')arg++;}else arg="";for(char*p=line;*p;p++)*p=toupper((unsigned char)*p);if(need_auth(line)&&!s->authed){reply(s,530,"Not logged in.");return true;}char v[FTP_PATH_MAX],p[FTP_PATH_MAX];p[0]=0;struct stat st;
 if(!strcmp(line,"USER")){snprintf(s->login_user,sizeof(s->login_user),"%s",arg);s->authed=g.anon&&!strcasecmp(arg,"anonymous");reply(s,s->authed?230:331,s->authed?"Anonymous login okay.":"Password required.");}
 else if(!strcmp(line,"PASS")){s->authed=(g.anon&&!strcasecmp(s->login_user,"anonymous"))||(!strcmp(s->login_user,g.user)&&!strcmp(arg,g.pass));reply(s,s->authed?230:530,s->authed?"Login successful.":"Authentication failed.");}
 ...
 else if(!strcmp(line,"TYPE"))reply(s,200,"Type set.");
 ...
 else if(!strcmp(line,"REST")){char*end;s->rest=strtoll(arg,&end,10);if(*end||s->rest<0){s->rest=0;reply(s,501,"Invalid offset.");}else reply(s,350,"Restart position accepted.");}
 ...
 return true;}
static void session_task(void*arg){session_t*s=arg;reply(s,220,"ESP32 FTP server ready.");char line[FTP_LINE_MAX];size_t used=0;bool stay=true;while(running&&stay){char c;ssize_t n=recv(s->ctrl,&c,1,0);if(n<=0)break;if(c=='\n'){if(used&&line[used-1]=='\r')used--;line[used]=0;if(used)stay=command(s,line);used=0;}else if(used+1<sizeof(line))line[used++]=c;else{used=0;reply(s,500,"Command line too long.");}}closefd(&s->pasv);closefd(&s->ctrl);free(s);xSemaphoreTake(lock,portMAX_DELAY);clients--;xSemaphoreGive(lock);vTaskDelete(NULL);}
